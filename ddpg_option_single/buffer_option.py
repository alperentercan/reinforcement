...
class Replay_buffer():
    def __init__(self,size,obs_size,action_size):
        ...
        self.width = obs_size + action_size + 1 + 1 + obs_size + 1 
        self.replay_buffer = torch.zeros([self.size,self.width],dtype=torch.float32)
        self.buff_ind = 0
        self.buff_curr_size = 0
        
    
    def add_entry(self,obs,a,w,reward,obs_new,done):
        done = 0 if done else 1
#         print('act ',a.shape)
#         print('obs ',obs.shape)
# #         reward = reward.reshape(0)
#         print(type(reward.squeeze()))
...
#         print(done.shape)
    ...
    def sample_split_batch(self,batch_size):
        ...
        return obs_batch,act_batch,w_batch,reward_batch,obs_new_batch,done_batch